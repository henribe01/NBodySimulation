...
def generate_particles(num_stars: int, center: tuple[float, float] = (0.0, 0.0), initial_velocity: tuple[float, float] = (0.0, 0.0),
                       min_radius: float = 0.2, max_radius: float = 2.0, mass_bulge: float = 0.8, mass_disk: float = 0.2) -> list[Particle]:
    """
    ...
    vx_disk = -v_mag * np.sin(angles) + initial_velocity[0]  # Add initial velocity to the disk particles
    vy_disk = v_mag * np.cos(angles) + initial_velocity[1]
...
if __name__ == "__main__":
    ...
    animation_time = 20  # Total time for the animation in seconds
    ...
    # Generate particles
    particles = generate_particles(num_stars, initial_velocity=(0.5, 0.0))
